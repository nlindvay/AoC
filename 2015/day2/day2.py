...
with(open("2015\day2\day2_input.txt", "r")) as F:
    ...
    while(True):
        ...
        feet += (2*l*w) + (2*w*h) + (2*l*h) + mn

    print("part 1 answer: ", feet)
...
with(open("2015\day2\day2_input.txt", "r")) as F:
    ...
    while(True):
        ...
        nums = line.split("x")
        feet += int(nums[0]) * int(nums[1]) * int(nums[2]) # add bow footage
        mx = max(int(nums[0]), int(nums[1]), int(nums[2])) # find max
        nums.remove(str(mx)) # remove max
        feet += (int(nums[0]) * 2) + (int(nums[1]) * 2) # add ribbon footage
        
    print("part 2 answer: ", feet)