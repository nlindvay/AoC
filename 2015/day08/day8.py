...
# are \\ (which represents a single backslash), \" (which represents a lone double-quote character), and \x plus two hexadecimal 
...
with(open("2015\day08\input.txt", "r")) as F:
    while True:
        ...
        newline = newline.replace(r"\"", "\"") # replace all \" with "
        newline = newline.replace("\\", "`") # replace all \\ with 
        newline = newline.replace(r"``", "\\") # replace all `` with \
        newline = re.sub("[`][x][a-f0-9][a-f0-9]", "+", newline)

        newCount += len(newline)
