...
with(open("2015\day02\input.txt", "r")) as F:
    ...
    while(True):
        ...
        # part 1
        mn = min(nums[0]*nums[1], nums[1]*nums[2], nums[2]*nums[0])
        fmin += (2*nums[0]*nums[1]) + (2*nums[1]*nums[2]) + (2*nums[2]*nums[0]) + mn

        # part 2 
        fmax += (nums[0] * nums[1] * nums[2]) # add bow footage
        mx = max(nums[0],nums[1],nums[2]) # find max
        nums.remove(mx) # remove max
        fmax += (nums[0] * 2) + (nums[1] * 2) # add ribbon footage

    print("part 1 answer: ", fmin)
    print("part 2 answer: ", fmax)
